...
struct Argon2Core {
  ...
  static func argon2Hash(password: UnsafeMutablePointer<Array<Int8>>, salt: String, params: [String: Any]) -> [UInt8] {
    // Initialize Argon2
    ...
    let hash = argon2Crypto.hash(passwordPointer: password)
    return hash.value!
  }
